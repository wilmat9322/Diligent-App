  ...
  if (empty($comp_code)) {
    ...
    header("Location: ../manage_admin.php?error=Earning rate is required&$user_data");
       ...
       $sql = "UPDATE users_table
               SET comp_code='$comp_code', name='$name', user_name='$user_name', position='$position', type_employee='$type_employee', work_time='$work_time', hours_work='$hours_work', earn_rate='$earn_rate', password='$pass'
               WHERE id=$id ";
       $result = mysqli_query($conn, $sql);
       if ($result) {
       	  header("Location: ../manage_admin.php?success=Employee Successfully Updated");
       }else {
