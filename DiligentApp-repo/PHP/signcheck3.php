...
if (isset($_POST['uname']) && isset($_POST['sec_num'])) {
    ...
    if(empty($uname)) {
      header("Location: register3.php?error=Employee Code is required");
      exit();

    }else if(empty($sec_num)) {
      header("Location: register3.php?error=SSN is required");
      exit();
  ...
  if (mysqli_num_rows($result) === 1) {
    ...
    if ($row['user_name'] === $uname && $row['sec_num'] === $sec_num) {
      ...
      header("Location: register3.php?error=Incorrect Code or SSN");
      exit();
    }
  }else{
    header("Location: register3.php?error=Incorrect Code or SSN");
